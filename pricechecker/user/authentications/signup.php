...
//        $email_check = "SELECT * FROM `admin` WHERE com_email = '$email'";
        $email_check = "SELECT * FROM users WHERE email = '$email'";
        $res = mysqli_query($con, $email_check);
        if(mysqli_num_rows($res) > 0){
            $response['status'] = 'failed';
            $response['error'] = "Email that you have entered is already exist!".$email_check;
        }else{
...
//            $insert_data = "INSERT INTO `admin` (username, com_email, password)
