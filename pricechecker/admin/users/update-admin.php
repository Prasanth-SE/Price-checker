...
if(isset($_POST['name']) && $_POST['email'] && $_POST['password'] && $_POST['id']){
    ...
    $strQuery = "select * from `admin` where `id`='$id'";
    $res=mysqli_query($con,$strQuery);
    if(mysqli_num_rows($res) === 1){
        $fetch = mysqli_fetch_assoc($res);
        $fetch_pass = $fetch['password'];
        if(password_verify($password, $fetch_pass)){
            $strQuery = "update `admin` set name='$name', email='$email' where id=$id";
        }else{
            $password = password_hash($password, PASSWORD_BCRYPT);
            $strQuery = "update `admin` set name='$name', email='$email',password='$password' where id=$id";
        }
