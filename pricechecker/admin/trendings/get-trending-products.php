...
if(isset($_GET['currentPage']) && isset($_GET['numberInPage'])){
    ...
    $strquery = "select * from `trending_products` where id > $pastPageLastId order by id desc limit $numberInPage";
    ...
    if(mysqli_num_rows($res) > 0){
        ...
        $response['status'] = 'failed';
        $response['error'] = 'There are no favourite products for you.'.$strquery;
    }
}else{
    $response['status'] = 'failed';
    $response['error'] = 'The parameter could not be parser.'.$_GET['currentPage'].$_GET['numberInPage'];
}
$response['data'] = $data;
