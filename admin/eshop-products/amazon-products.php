...
$data = array();
    if(isset($_GET['currentPage']) && isset($_GET['numberInPage'])){
        ...
        if($err){
            ...
            $response['status'] = 'ok';
            $data = convertAmazon2Response($mockData, $currentPage, $numberInPage);
        }else{
            $response['status'] = 'ok';
            $response['error'] = 'success in curl';
            $array = json_decode($result);
            $data = convertAmazon2Response($array, $currentPage, $numberInPage);
        }
    }
...
function convertAmazon2Response($response, $currentPage, $numberInPage){
    $res = array();
